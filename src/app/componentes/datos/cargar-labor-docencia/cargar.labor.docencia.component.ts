...
export class CargarLaborDocenciaComponent implements OnInit {
  ...
  cargarProgramas() {
    this.programaService.consultarProgramasPermitidosPorUsuario().subscribe(
      ...
      (error) => {
        console.error(error);
        this.messageService.showMessage("error", "Error al cargar los programas");
      }
    );
  }
  ...
  guardar() {
    ...
    this.http.get<any[]>('/assets/mock-labor-docente.json').subscribe(
        (docenteLaborList) => {
            this.sharedService.importarLaborDocente(docenteLaborList, 1, this.programa.idPrograma).subscribe(
                (response) => {
                    this.isLoading = false;
                    this.messageService.showMessage("success", "Labor docente importada correctamente");
                ...
                (error) => {
                    ...
                    let mensajeError = "Ocurrió un error inesperado al importar la labor docente.";
        ...
        (error) => {
            ...
            this.messageService.showMessage("error", "Error al cargar el archivo JSON de prueba.");
        }
    );
}
    ...
    if (this.programa) {
      ...
      this.sharedService.obtenerVisualizarLaborDocente(filtro).subscribe(
        (response) => {
          ...
          this.messageService.showMessage("success", "Visualización de la labor docente realizada correctamente");
        },
        (error) => {
          console.error('Error al visualizar:', error);
          this.messageService.showMessage("error", "Error al visualizar la labor docente");
        }
      );
    } else {
      this.messageService.showMessage("error", "Por favor, seleccione un programa");
    }
  }
